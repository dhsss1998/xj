...
import jee.support.dao.RewardDao;
...
public class StudentService {
    ...
    public long addStudent(@NotNull Student student) {
        ...
        studentDao.addStudent(student);// 插入票据记录,获取票据id
    ...
    public  List<Reward> selectuser(){
        return studentDao.selectuser();
    ...
    public int delStudent(long studentId) {
        return studentDao.delStudent1(studentId);
    }
...
//动态查询
public List<Score> findByStudentId(long Id){
    return scoreDao.findByStudentId(Id) ;
}
...
//    返回受影响的行数
    @Transactional
    public int  updateStudent(long id, @NotNull Student student) {
...
//
    ...
    @Transactional
    public int delStudentReward(long studentId) {return studentDao.delStudentReward(studentId);
    }
    @Transactional
    public int addStudentReward(@NotNull Reward reward) {
